...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for field in PLAYERBATTINGCAREER_FIELDS:
            ...
            if fieldName != 'Player':
                if fieldName not in PER_AB_FIELDNAMES:
                    
                    print('calculating ' + field['longName'] + ' percentiles...')
                
                    pTiles = [0] * 100
                    gteOneFilter = {fieldName + '__gte': 1}
                    ...
                    for i in range(100, 0, -1):
                        ...
                        pTileMinPlayer = orm.PlayerBattingCareer.objects.order_by('-'+fieldName)[pTileMinIndex-1:pTileMinIndex].values()
                    
                        pTileMinValue = pTileMinPlayer[0][fieldName]
                        pTiles[i-1] = pTileMinValue
                    
                        pTileMinIndex += pTileSize
                
                    ptileDict[fieldName] = pTiles
                
                else:
                    
                    print('calculating ' + field['longName'] + ' per AB percentiles...')
                
                    pTiles = [0] * 100
                    gteTenFilter = {'AtBats__gte': 10}
                    numGoodCareers = orm.PlayerBattingCareerPerAB.objects.filter(AtBats__gte=10).count()
                    ...
                    for i in range(100, 0, -1):
                        if i == 1:
                            pTileMinIndex = orm.PlayerBattingCareerPerAB.objects.filter(AtBats__gte=10).count() - 1
                    
                        pTileMinPlayer = orm.PlayerBattingCareerPerAB.objects.filter(AtBats__gte=10).order_by('-'+fieldName)[pTileMinIndex-1:pTileMinIndex].values()
